          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <View>
              <View style={globals.setImgHead}>
                ...
                <TouchableOpacity
                  style={[styles.setImgButton, globals.alCenter]}
                  onPress={() => setCover(0)}
                >
                  <Text style={styles.setImgBtnTxt}>Padrão</Text>
                </TouchableOpacity>
              </View>
              <View style={globals.setImgDesc}>
              ...
              <View style={styles.setImgViewButtons}>
                <View style={styles.setImgButtons}>
                  <TouchableOpacity
                    ...
                    onPress={() => setCover(1)}
                  >
                    <Text style={styles.setImgBtnTxt}>Conter</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.setImgButton, globals.alCenter]}
                    onPress={() => setCover(2)}
                  >
                    <Text style={styles.setImgBtnTxt}>Esticar</Text>
                  </TouchableOpacity>
                </View>
                <View style={styles.setImgButtons}>
                  <TouchableOpacity
                    ...
                    onPress={() => setCover(3)}
                  >
                    <Text style={styles.setImgBtnTxt}>Repetir</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.setImgButton, globals.alCenter]}
                    onPress={() => setCover(4)}
                  >
                    <Text style={styles.setImgBtnTxt}>Centralizar</Text>
                  </TouchableOpacity>
                </View>
              </View>
            </View>
            ...
            <View>
              ...
              <View style={image ? styles.test : styles.setNameView}>
                {image && (
                  <Image
                    source={{ uri: image }}
                    style={[styles.imageSets, { resizeMode: "contain" }]}
                  />
                )}
                <TouchableOpacity
                  style={[
  ...
  const renderGroupList = () => {
    return data.data.map((group, index) => (
      <View key={index} style={styles.itemList}>
        <TouchableOpacity
          activeOpacity={0.8}
          onPress={() => navigation.navigate("List", { id: group.id, name: group.name })}
        >
          <Image
            style={[styles.itemImg, { resizeMode: IMAGESET[group.imageSet] }]}
            source={{ uri: group.image }}
          />
        </TouchableOpacity>
        {/*Engrenagem de configuração  */}
        <TouchableOpacity
          style={[styles.itemGear, globals.alCenter]}
          onPress={() => {
            setTmpList({ name: group.name, dataPos: index });
            showGroupConfigWindow
              ? setShowGroupConfigWindow(false)
              ...
              <View>
                <View style={globals.saveSetView}>
                  ...
                  >
                    <Icon
                      name="check"
                      color={canSaveGroup ? theme.d : theme.darkNegate}
                      size={30}
                    />
                  </TouchableOpacity>
  ...
  const dataCanSaveGroup = (name, uri) => {
    if (name == "") {
      ...
      data.data.push({
        ...
        name,
        imageSet: 0,
        image: uri != null ? uri : "./../../assets/icon.png",
      });
      console.log(data);
      saveData(data);
      return true;
  ...
  const createJSONArchive = async (JSONContent) => {
    ...
    try {
      await StorageAccessFramework.createFileAsync(
        ...
        .then(async (uri) => {
          await FileSystem.writeAsStringAsync(uri, JSON.stringify(JSONContent), {
